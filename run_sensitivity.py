...
def run_global_sensitivity(
    python_executable: str, simulation_script: Path, output_root: Path,
    n_samples: int, seeds: Iterable[int], skip_existing: bool, dry_run: bool
) -> None:
    """全局敏感性分析：同时随机改变多个参数 (Monte Carlo Random Sampling)"""
    scenario = "supply_adjusted" # 通常全局SA在最完整的机制下测试
    ...
    # 如果已有日志且开启skip_existing，读取旧配置；否则生成新配置
    if skip_existing and params_log_path.exists():
        ...
        for i in range(n_samples):
            samples.append({
                ...
                "riders": random.randint(100, 300),
                "heatmap_alpha": round(random.uniform(0.0, 2.0), 2),
                "heatmap_beta": round(random.uniform(0.0, 2.0), 2),
                "heatmap_gamma": round(random.uniform(0.0, 2.0), 2),
            })
        if not dry_run:
...
def main() -> None:
    ...
    if "global" in experiments:
        run_global_sensitivity(args.python_executable, args.simulation_script, args.output_root, args.global_samples, seeds, args.skip_existing, args.dry_run)
