...
def run_global_sensitivity(
    ...
    if skip_existing and params_log_path.exists():
        with open(params_log_path, "r", newline="") as f:
    ...
    for sample in samples:
        ...
        for seed in seeds:
            ...
            cmd = [
                ...
                str(sample["riders"]),
                "--heatmap-alpha",
                str(sample["heatmap_alpha"]),
                "--heatmap-beta",
                str(sample["heatmap_beta"]),
                "--heatmap-gamma",
                str(sample["heatmap_gamma"]),
                "--seed",
                str(seed),
                "--output",
                str(output_path),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run local and lightweight global sensitivity analysis for the delivery-rider ABM."
    )
    parser.add_argument(
        "--experiments",
        nargs="+",
        choices=["riders", "heatmap_beta", "global", "all"],
        default=["all"],
    )
    parser.add_argument(
        "--n-seeds",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of random parameter combinations for lightweight global SA.",
    )
    parser.add_argument(
        "--sample-seed",
        type=int,
        default=12345,
        help="Seed used to generate global sensitivity parameter samples.",
    )
